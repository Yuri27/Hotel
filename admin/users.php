		...
		<div class="wrapper">
			...
			<header>
				<form id="top_nav" name="search" action="bron.php" method="GET" onsubmit="myButton.disabled = true; return true;">
					...
					<input class="services" type="text" name="" value="Пользователи сайта" disabled="disabled">
				</form>
			...
			<article>
				...
				<form action="users.php" method="POST" enctype=multipart/form-data>
					...
					<button id="save_btn" type="submit" name="block">Заблокировать</button>
					<button id="save_btn" type="submit" name="unblock">Разблокировать</button>


				<? 
				echo '<table class="simple-little-table" cellpadding="5" cellspacing="0" style="width:100%;" align=center>';
				echo '<tr><th><a>Название</a></th>
							<th><a>Количество номеров</a></th>
							<th><a>Количество комнат</a></th>
							<th><a>Цена</a></th>
							<th><a>О категории</a></th>
							<th><a>Заблокирован</a></th>
							<th><a>Фото</a></th></tr>';
				foreach ($data as $row){
						echo '<tr>';
						echo '<td><input type="text" name="name_category" value="'.$row['name'].'" ></td>';
						echo '<td><input type="text" name="number_rooms" value="'.$row['login'].'"></td>';
						echo '<td><input type="text" name="quantity_rooms" value="'.$row['email'].'"></td>';
						echo '<td><input type="text" name="cost" value="'.$row['phone'].'"></td>';
						echo '<td><input type="text" name="cost" value="'.$row['signup_date'].'"></td>';
						if ($row['status']=='Y')
							echo '<td><a><input type="checkbox" name="status[]" value='.$row['id'].'></input></a></td>';
						else
							echo '<td><a><input type="checkbox" name="status[]" value='.$row['id'].' checked></input></a></td>';

						echo '<td><img src=../'.$row['avatar'].' </td>';
						// echo '<td><input type="text" name="cost" value="'.$row['status'].'"></td>';
